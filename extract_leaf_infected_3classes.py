...
def main():
    ...
    # 加载三分类模型（只加载一次）
    print("\n加载三分类模型...")
    model, scaler, label_encoder, device = load_three_classes_model()
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('.hdr'):
            ...
            infected_mask, class_counts = filter_infected_pixels(
                hyper_data, mask, model, scaler, label_encoder, device
            )
            # 保存三分类过滤后的感染叶片bmp
            ...
            if infected_mask.sum() > 0:
                infected_data = hyper_data * np.expand_dims(infected_mask, axis=2)
                out_base_infected = os.path.join(output_dir, base+'_infected')
                save_hyper_data(infected_data, metadata, out_base_infected)
                print(f"✓ 感染叶片像素: {infected_mask.sum()} / {mask.sum()} ({infected_mask.sum()/(mask.sum()+1e-6)*100:.1f}%)")
            else:
                print(f"⚠ 未检测到感染叶片像素，跳过保存")
            print(f"{'='*60}\n")
